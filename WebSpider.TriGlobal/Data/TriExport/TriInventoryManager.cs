...
{
    public class TriInventoryManager : DataManager
    {
        #region [Constructror]
        public TriInventoryManager(string ConnectionString)
        {
        ...
        #region [ Get Data ]
        public List<Tri_Inventory> GetData()
        {
            String Query = "SELECT * FROM TriInventory";
            OleDbDataManager oDm = new OleDbDataManager(ConnectionString, Query, true);
            return DataParser.ToList<Tri_Inventory>(oDm.GetTable());
        }

        public List<Tri_Inventory> GetData(String PART_NUM)
        {
            String Query = "SELECT * FROM TriInventory WHERE PART_NUM = \"" + PART_NUM + "\"";
            OleDbDataManager oDm = new OleDbDataManager(ConnectionString, Query, true);
        ...
        #region [ Save ]
        public int Save(Tri_Inventory TriInventory)
        {
            return Save(TriInventory.ID, TriInventory.PART_NUM, TriInventory.TotalInventory, TriInventory.Dallas, TriInventory.DC_AtlantaHub
                , TriInventory.DC_Dallas_Hub, TriInventory.DC_Elk_Grove_Hub, TriInventory.DC_Feura_Bush, TriInventory.DC_Louisville_Hub
                , TriInventory.DC_Reno_Hub, TriInventory.DC_Richmond_Dist_Ctr, TriInventory.Oklahama, TriInventory.RemainingBranches, TriInventory.LastUpdate);
        }
        ...
        {
            String Query = "INSERT INTO TriInventory (PART_NUM, TotalInventory, Dallas, DC_AtlantaHub, DC_Dallas_Hub, DC_Elk_Grove_Hub, DC_Feura_Bush, DC_Louisville_Hub, DC_Reno_Hub, DC_Richmond_Dist_Ctr, Oklahama, RemainingBranches, LastUpdate)"
        ...
        public int Update(long ID, string PART_NUM, int? TotalInventory, int? Dallas, int? DC_AtlantaHub, int? DC_Dallas_Hub, int? DC_Elk_Grove_Hub
            , int? DC_Feura_Bush, int? DC_Louisville_Hub, int? DC_Reno_Hub, int? DC_Richmond_Dist_Ctr, int? Oklahama, int? RemainingBranches, DateTime? LastUpdate)
        {
            String Query = "UPDATE TriInventory SET TotalInventory = @TotalInventory, Dallas = @Dallas, "
        ...
        {
            String Query = "DELETE FROM TriInventory SET WHERE PART_NUM = \"" + PART_NUM + "\"";
            OleDbDataManager oDm = new OleDbDataManager(ConnectionString, Query, true);
            //oDm.AddVarcharPara("PART_NUM", 4000, PART_NUM);
            return oDm.RunActionQuery();
        }
        ...
        {
            String Query = "DELETE FROM TriInventory SET WHERE PART_NUM = \"" + PART_NUM + "\" AND PropertyName = \"" + PropertyName + "\"";
            OleDbDataManager oDm = new OleDbDataManager(ConnectionString, Query, true);
            //oDm.AddVarcharPara("PART_NUM", 4000, PART_NUM);
